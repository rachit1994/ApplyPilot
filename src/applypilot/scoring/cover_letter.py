...
from applypilot.scoring.validator import (
    ...
    validate_workatastartup_message,
)
...
def _build_workatastartup_message_prompt(profile: dict) -> str:
    personal = profile.get("personal", {})
    name = personal.get("preferred_name") or personal.get("full_name", "the candidate")
    roles = ", ".join(get_target_roles(profile))
    all_banned = ", ".join(f'"{w}"' for w in BANNED_WORDS)
    return f"""Write a Work at a Startup application message as {name} (first person).
Candidate is pursuing: {roles}.

This is NOT a cover letter. No "Dear Hiring Manager". No subject line. No sign-off block.
Write 3-5 short sentences (under 150 words) the founder reads in the apply box.

Structure:
1. One line: why this specific role at this company (reference something concrete from the job post).
2. One proof point from the resume with a real metric.
3. One line on fit (stack, domain, or scale) tied to what they are building.
4. Close with availability to talk — direct, not salesy.
...
def generate_cover_letter(
    ...
    cl_prompt_base = (
        _build_workatastartup_message_prompt(profile)
        if waas
        else _build_cover_letter_prompt(profile)
    )
    user_task = (
        "Write the application message:"
        if waas
        else "Write the cover letter:"
    )
    ...
    for attempt in range(max_retries + 1):
        ...
        validation = (
            validate_workatastartup_message(letter, mode=validation_mode)
            if waas
            else validate_cover_letter(letter, mode=validation_mode)
        )
