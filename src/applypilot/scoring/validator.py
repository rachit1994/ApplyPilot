...
def validate_workatastartup_message(text: str, mode: str = "normal") -> dict:
    """Validate a short Work at a Startup founder application message."""
    errors: list[str] = []
    warnings: list[str] = []
    text_lower = text.lower()
    ...
    words = len(text.split())
    if mode == "strict" and words > 180:
        errors.append(f"Too long ({words} words). Max 180.")
    elif mode == "normal" and words > 200:
        warnings.append(f"Long ({words} words). Target under 150.")
    ...
    if len(text.strip()) < 80:
        errors.append("Message too short — needs role-specific proof.")
