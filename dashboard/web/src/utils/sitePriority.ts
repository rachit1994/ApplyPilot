...
export const PRIORITY_BOARD_NAMES = ["Naukri", "Wellfound"] as const;

export const APPLY_QUEUE_ORDER_LABEL =
  "Naukri → Wellfound → ATS boards → other";

export function isPriorityBoardName(name: string | null | undefined): boolean {
  const lowered = (name ?? "").trim().toLowerCase();
  if (!lowered) return false;
  if (lowered === "naukri" || lowered.includes("naukri")) return true;
...
export function prioritySortKey(name: string | null | undefined): number {
  ...
  if (idx >= 0) return idx;
  if (lowered.includes("naukri")) return 0;
